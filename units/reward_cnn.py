...
FilePath: //ebike_trajectory_prediction//units//reward_cnn.py
"""
import numpy as np
...
class FunctionNet(Model):
    def __init__(self, num_inputs, hidden_dim, output=1, init_w=3e-3):
        ...
        self.linear3 = Dense(n_units=output, W_init=w_init, in_channels=hidden_dim, name='f3')
...
class DsFunctionNet(Model):
    def __init__(self, num_inputs, hidden_dim, output=1, init_w=3e-1):
        super(DsFunctionNet, self).__init__()
        # init weight
        w_init = tf.random_uniform_initializer(-init_w, init_w)
        # three dense layer
        # self.inputlayer = Input(shape=[num_inputs,],dtype=tf.float32)
